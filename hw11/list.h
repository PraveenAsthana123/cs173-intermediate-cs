...
private:
    Node<ItemType>  *head,                                      // head of the linked list
    ...
    Node<ItemType>*  _find       (int index);                    // return a pointer to the node in position index
    
    friend ostream&    operator<<  (ostream& os, const List& l)
	{
	  Node<ItemType> *temp = l.head;			// Create a string to hold value
	  cout << "[";
	  while (temp != NULL) 					// Append number to string
	    {
	      if (temp->next != NULL)
		{
		  os << temp->item << ", ";
...
public:
    StopIteration() {};
    ~StopIteration() {};
    const char *Reason () const {return "Iteration goes off bound.";}
};
